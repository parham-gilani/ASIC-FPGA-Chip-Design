...
int main()
{
	...
	while(1)
	{
		...
		{
			Gpio_flag = 0 ;
			xil_printf("Please provide start address\t\n") ;
			Start_Addr = 0;
			xil_printf("Start address is %d\t\n", Start_Addr) ;
			xil_printf("Please provide length\t\n") ;
			//scanf("%d", &Len) ;
			Len = 1024;
...
int bram_read_write()
{
	...
	{
		tmp = sin(8000 * 2 * M_PI * time);
		Write_Data = data_to_u32(tmp);
		time += time_inc;
